...
function touchMove(event) {
  ...
  if (
    ...
    progress = Math.trunc(/* visuallyItem / 2 +  */ drumSide.length / 2) * step;
    rotateX =
      Math.trunc(/* visuallyItem / 2 +  */ drumSide.length / 2) * step +
      inititalRotateX;
  }
  ...
  if (drumSide.length > 9) {
    ...
    if (progress < 0) {
      ...
      drumSide.forEach((item, index) => {
        if (index >= indexFromEnd) {
          drumSide[index].classList.add("repeated");
          drumSide[index].style.transform = `rotateX(${
            35 + step * (drumSide.length - 1 - index) + step
            }deg) translateZ(100px)`; // сделать так, чтобы элементы вставлялись перед первым
        } else {
          drumSide[index].classList.remove("repeated");
          item.style.transform = `rotateX(${
            -(index * step) + startDegSide
      ...
      if (Math.abs(progress) > 9 * step) {
        ...
        progress = (drumSide.length - 1 - visuallyItem + 1) * step;
        drumContainer.style.transform = ` rotateY(3deg) rotateX(${
          30 + progress
          }deg)`;
    ...
    drumSide.forEach((item, index) => {
      if (
        (index * step >= progress && index * step < progress + visuallyZone) ||
